...
\textbf{Why are 2 measuring tapes of $H_{max}$ and $H_{max}+20cm$ required?}
\begin{itemize}
\item We use a $H_{max}$ ruler to measure the height of the robot.
  % TODO: Are we using this device to measure Htop?
  % - If not, this should be changed,
  % - If we do, it might be worth adding the 'measuring htop' part from the
  %   original document
  % In both cases, this will need to be adjusted
\item $H_{max}+20cm$ is used to determine the centre of gravity by moving the Aluminium
  Plate up or down.
  % TODO: This is not according to what is shown in fig:com-measurement-4,
  %   it seems the H_max ruler is used and therefore this one is useless
\end{itemize}
...
\item At this position, the wooden board should be slanted 45 degrees from a
  flat surface.
  % TODO: investigate on this sentence which sounds really weird
  % This 45 degrees sounds really suspicious to me since the height of the pipe
  % straps is much smaller than the half-length of the wooden board
