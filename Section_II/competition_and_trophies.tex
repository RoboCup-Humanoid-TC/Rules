...
\added{Aside of taking physical measurements, all robots must participate in a set of benchmark tests to prove their abilities.
Performance of robots in these tests is recorded to track the development of the league over the years.
Entry requirements may be adjusted each year during the rulebook creation depending on the performances in the previous years.
The following benchmarking tests must be performed:
\begin{itemize}
	\item Movement Speed: The robot must walk from one touch line to the other touch line. If the robot falls, it needs to recover from the fall autonomously. The time to cross the field is recorded. To qualify, the robot must walk at least 3 m in one minute.
	\item Kick Distance: The robot must kick a ball from the center point of the field. The kick distance is recorded. To qualify, the ball must be kicked at least 30 cm.
	\item Get-up Time: The robot must stand up from both the prone and supine position. The time to stand up is recorded. To qualify, each stand-up motion must take less than 30 seconds. In AdultSize, this test is not mandatory, but teams are encouraged to demonstrate their capabilities. AdultSize teams do not incur any penalty for not passing this test.
	\item Ball Detection Distance: The robot is placed in the goal and a randomly chosen ball is gradually moved away from the robot until it can no longer be recognized. The distance between the ball and the robot is measured. To qualify, the robot must be able to recognize the ball.
	\item Ready Phase Placement: The robot is placed on a random sideline at the penalty line. When the game controller gives the ready signal, the robot must start placing itself as the kicker in the center circle. The distance to the kick-off position at the end of the ready phase is recorded. To qualify, the robot must be inside the field at the end of the ready phase.
\end{itemize}}
