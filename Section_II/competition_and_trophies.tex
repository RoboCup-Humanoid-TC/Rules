...
\item must not touch any robot. Touching a robot is considered as an offense that is penalised by free kick or penalty kick according to the laws of the game.
...
\item \textcolor{magenta}{After a period between 15 and 30 seconds the referee gives the signal ``SET''. The referee calls robots illegally positioned to be removed with exception of the goal keeper and a striker for kick-off that may be placed manually by the respective robot handler. Removed players are placed on the touch line of the respective team's side, facing the opposite touch line, to enter the field from there upon the ``PLAY'' signal. } 
\item  \textcolor{blue}{\sout{The opponents of the team taking the kick-off are outside the center circle until the ball is in play.}}
...
Robots being able to autonomously reposition themselves can take any position on the field that is consistent with above requirements. \textcolor{magenta}{It is not allowed to manually (re-)position an autonomously positioned robot that took a legal position, except for the goal keeper and the striker for kick-off or drop ball, if applicable.}
...
\textcolor{magenta}{Similar to a kick-off situations one striker robot for each team may be placed manually for a drop ball, if both teams can not position themselves automatically}.
...
Drop-in players are initially ranked according to the accumulated points. If there are ties, the tied players are ranked according to the geometric average of their points. The three most highly ranked players of a sub-league receive a 'Best Player' certificate.

If the three best drop-in players can not be identified with the scheme provided above, an additional game with players dawn group wise (from the groups of equally high ranked players) from the highest-ranked players is to be played.
