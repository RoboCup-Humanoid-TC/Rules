...
period. Teams need to submit a first draft of a file containing their robot model \removed{two months} \
...
\item must not enter the radius of \removed{one arm length}\added{1.5 meters} around the robot unless the robot is to be picked up or to avoid interference with the game. Violation of this rule results in a warning to the respective robot handler. After two warnings, the robot handler needs to be replaced similarly to the rule specified under ``Request for Pick-up''. \added{Entering the radius further causes the robot to be picked up, unless this would give an advantage to the offending team.}
...
\added{For the software challenge the winner receives 30 points. The second best team receives 21 points. The third best team receives 15 points.}
...
\added{A certificate is awarded to the team that performed best in the software challenge.}
