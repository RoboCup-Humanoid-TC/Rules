...
\bigskip
A penalty kick is awarded against a team that commits one of the \added{six} \greyed{(replaces: ten}) offences for which a direct free kick is awarded, inside its own penalty area and while the ball is in play.
...
\begin{itemize}
\item  \added{must be placed on the penalty mark}. \removed{is placed at the position where the offence occurred}
\end{itemize}

\added{(new:) During penalty shoot-out, the player taking the penalty kick:}

\begin{itemize}
\item \added{must be properly identified}
\end{itemize}
...
The players other than the kicker must be: \removed{located at least 0.5 m from the ball and not anywhere between the current ball position and the goal.}

\bigskip
\begin{itemize}
\item \added{inside the field of play}
\item \greyed{(suspended: outside the penalty area)}
\item \added{behind the penalty mark}
\item \added{at least 0.75m for KidSize and TeenSize and 1.5m for AdultSize from the penalty mark} \greyed{(replaces: 9.15m)}
\end{itemize}
...
\added{If a penalty kick is taken during the normal course of play} the same procedure as in regular direct free kicks is applied.

\bigskip
\added{
During penalty shoot-out:}
\begin{itemize}
\item \added{After the players have taken positions in accordance with this law, the
referee signals for the penalty kick to be taken}
\item \added{The player taking the penalty kick must kick the ball forward}
\item \greyed{(suspended: He must not play the ball again until it has touched another player)}
\item \added{The ball is in play when it is kicked and moves forward}
\end{itemize}
...
\added{The trial ends after 60 seconds. It may be extended until the ball comes to a complete stop if the ball is still moving at the time the 60 seconds are over. The trial also ends if the ball stops \added{being entirely} inside the \added{goal area} \removed{marked area around the goal} or leaves the field.}
