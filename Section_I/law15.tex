...
A throw-in is a method of \removed{continuing} \added{restarting} play.
...
\added{A goal cannot be scored directly from a throw-in:}
\begin{itemize}
\item \added{if the ball enters the opponents' goal - a goal kick is awarded}
\item \added{if the ball enters the thrower's goal - a corner kick is awarded}
\end{itemize}
...
If the ball leaves the field it will be replaced on the field by the referee or an assistant referee. \removed{There is no stoppage in play.} \added{If the whole of the ball passes over a touch line the ball is placed on the touch line directly at the point at which the ball left the field.}

\bigskip

\removed{The positions for replacement of the ball are described in the following with respect to the touch lines and always meant to be in a distance of about 40 cm orthogonal to the position on the touch line and
inwards to the playing field. }

\removed{If the whole of the ball passes over a touch line then the assistant
...
\begin{itemize}
\item \removed{If the referee cannot determine which robot was the last to touch
the ball before it left the field, then the ball is replaced directly
in from the point at which the ball left the field. }
\item \removed{Otherwise, the ball is placed one meter back from the point it
went out, where ``back'' is defined as being towards the goal of the team that last touched the ball. }
\end{itemize}

\removed{In any case, the ball cannot be placed closer than the length of the
...
\removed{After placing the ball, the ball is in play immediately.}
...
\begin{itemize}
\item \added{faces the field of play}
\item \added{has part of each foot either on the touch line or on the ground outside
the touch line}
\item \added{holds the ball with both hands}
\item \added{delivers the ball from behind and over his head}
